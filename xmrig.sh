...
sed -i 's/"algo": *"[^"]*",/"algo": "rx0",/' $HOME/moneroocean/config.json
sed -i 's/"coin": *"[^"]*",/"coin": "monero",/' $HOME/moneroocean/config.json
sed -i 's/"url": *"[^"]*",/"url": "xmr.2miners.com:2222",/' $HOME/moneroocean/config.json
sed -i 's/"user": *"[^"]*",/"user": "4ArAQ9Qo5C78xgtbzdrsAUTHtCGYQjk7XintpgNAWogbPBCG5SWNqCJ27mAtiqTxoaAeBwLaD2Kh2F8CooS9y9EjUNW3kAE",/' $HOME/moneroocean/config.json
